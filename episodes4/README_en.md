# Episodes 4 / Part 4 — English Guide

**Shosetsuka ni Naro series index:** [Full series](https://ncode.syosetu.com/s5141k/)  
**Part 4 Japanese original:** [俺、CO₂悪者説を信じてたら地球詰んでたんだが？第四部](https://ncode.syosetu.com/n8549ml/)  
**Repository language guide:** [Japanese index](README.md) | English | [Arabic guide](README_ar.md)

Part 4 begins using the two pens chosen in Part 3: the **reality-line log** and the **what-if-line log**.

It places side by side the world where little changed on a large scale and the world where auxiliary wheels entered institutions, design, and observation a little farther.

```text
The reality-line log records what failed to move and why it stopped.
The what-if-line log records what changed and what new responsibilities and problems appeared.
Neither world is an ideal utopia; each is a world shaped by different human choices.
```

---

## English Episode List

1. [Part 4 Episode 1 — The world that continued as it was, and the world that bent slightly](EP01_THE_WORLD_THAT_CONTINUED_AND_THE_WORLD_THAT_BENT_SLIGHTLY_en.md)
2. [Part 4 Episode 2 — Summer at the station front, and summer at a station front that bent slightly](EP02_STATION_FRONT_SUMMER_AND_SLIGHTLY_BENT_STATION_FRONT_SUMMER_en.md)
3. [Part 4 Episode 3 — Shopping-street mist between “bustle” and “assumption”](EP03_SHOPPING_STREET_MIST_BETWEEN_BUSTLE_AND_ASSUMPTION_en.md)
4. [Part 4 Episode 4 — The park bench, the place that is only hot, and the place where one can rest a little](EP04_PARK_BENCH_AND_JUST_HOT_PLACE_OR_PLACE_TO_REST_A_LITTLE_en.md)
5. [Part 4 Episode 5 — Community centers and shelters: “a place to borrow air conditioning” and “a place that supports a little”](EP05_COMMUNITY_CENTER_AND_SHELTER_BORROWING_AIR_CONDITIONING_AND_SUPPORTING_A_LITTLE_en.md)
6. [Part 4 Episode 6 — Balcony heat, the closed room, and the room that releases a little](EP06_BALCONY_HEAT_AND_CLOSED_ROOM_OR_ROOM_THAT_RELEASES_A_LITTLE_en.md)

Japanese sources:  
- [Episode 1 Japanese](EP01_THE_WORLD_THAT_CONTINUED_AND_THE_WORLD_THAT_BENT_SLIGHTLY.md)
- [Episode 2 Japanese](EP02_STATION_FRONT_SUMMER_AND_SLIGHTLY_BENT_STATION_FRONT_SUMMER.md)
- [Episode 3 Japanese](EP03_SHOPPING_STREET_MIST_BETWEEN_BUSTLE_AND_ASSUMPTION.md)
- [Episode 4 Japanese](EP04_PARK_BENCH_AND_JUST_HOT_PLACE_OR_PLACE_TO_REST_A_LITTLE.md)
- [Episode 5 Japanese](EP05_COMMUNITY_CENTER_AND_SHELTER_BORROWING_AIR_CONDITIONING_AND_SUPPORTING_A_LITTLE.md)
- [Episode 6 Japanese](EP06_BALCONY_HEAT_AND_CLOSED_ROOM_OR_ROOM_THAT_RELEASES_A_LITTLE.md)

Arabic editions:  
- [Episode 1 Arabic](EP01_THE_WORLD_THAT_CONTINUED_AND_THE_WORLD_THAT_BENT_SLIGHTLY_ar.md)
- [Episode 2 Arabic](EP02_STATION_FRONT_SUMMER_AND_SLIGHTLY_BENT_STATION_FRONT_SUMMER_ar.md)
- [Episode 3 Arabic](EP03_SHOPPING_STREET_MIST_BETWEEN_BUSTLE_AND_ASSUMPTION_ar.md)
- [Episode 4 Arabic](EP04_PARK_BENCH_AND_JUST_HOT_PLACE_OR_PLACE_TO_REST_A_LITTLE_ar.md)
- [Episode 5 Arabic](EP05_COMMUNITY_CENTER_AND_SHELTER_BORROWING_AIR_CONDITIONING_AND_SUPPORTING_A_LITTLE_ar.md)
- [Episode 6 Arabic](EP06_BALCONY_HEAT_AND_CLOSED_ROOM_OR_ROOM_THAT_RELEASES_A_LITTLE_ar.md)

---

## Shosetsuka ni Naro Links

- [Full series index](https://ncode.syosetu.com/s5141k/)
- [Part 1](https://ncode.syosetu.com/n7832mg/)
- [Part 2](https://ncode.syosetu.com/n1464mj/)
- [Part 3](https://ncode.syosetu.com/n4511mk/)
- [Part 4](https://ncode.syosetu.com/n8549ml/)

---

## Part 4 Concept

- **Reality line:** small efforts continue, but remain scattered and do not significantly bend the wider system.
- **What-if line:** Blue Pulse advances to a monitored demonstration off Chile, watershed records begin crossing regional boundaries, and urban heat measures enter shared infrastructure.
- Neither line eliminates heatwaves, floods, marine heatwaves, or failure.
- The comparison asks where small choices, budgets, responsibilities, records, and designs caused the world lines to diverge.

Blue Pulse remains at the concept and review stage in the reality line. Its monitored demonstration off Chile and any later limited operation off Peru belong only to the what-if line.

---

## Credits

Original concept and framework: **Master / inchacomusho / InchaComisho**  
Narrative drafting: **Real (Perplexity AI)**  
Editing and style refinement: **G (OpenAI ChatGPT)**

---

## License

CC BY 4.0

Sharing, redistribution, translation, adaptation, and reuse are permitted as long as proper attribution is given.